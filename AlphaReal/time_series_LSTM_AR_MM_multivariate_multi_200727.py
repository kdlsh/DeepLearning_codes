...
def build_multi_step_train_val_data(df):
    ...
    x_train_multi, y_train_multi = multivariate_data(
        dataset, dataset[:, 1], 0, TRAIN_SPLIT, past_history, future_target, STEP
    )
    x_val_multi, y_val_multi = multivariate_data(
        dataset, dataset[:, 1], TRAIN_SPLIT, None, past_history, future_target, STEP
    )
...
multi_step_model.add(
    tf.keras.layers.LSTM(
        12, return_sequences=True, input_shape=x_train_multi.shape[-2:]
    )
)

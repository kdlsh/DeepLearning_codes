...
def get_path_list():
    ...
    ## percent type
    path4 = (
        "D:\\workspace\\DeepLearning_codes\\AlphaReal\\raw_data\\JSratio_percent.txt"
    )
...
def build_merged_df(path_li, window_li, norm_flag_li, drop_reg_li, save_flag):
    ...
    df_final = reduce(
        lambda left, right: pd.merge(left, right, on="Date Time", how="outer"),
        df_roll_list,
    )
...
def build_pair_corr_table(
    cat1, start1, end1, cat2, start2, end2, corr_cutoff, drop_reg_li
):
    path_li, window_li, norm_flag_li = config()
    ...
    for win1 in range(start1, end1):
        for win2 in range(start2, end2):
            path_li, window_li, norm_flag_li = pair_data_config(
                path_li, norm_flag_li, cat1, win1, cat2, win2
            )
            df_merged, header_list = build_merged_df(
...
def plot_pair_pearson_corr(df, cat1, cat2, filename_prefix):
    ...
    rolling_r = (
        df_interpolated[cat1]
        .rolling(window=r_window_size, center=True)
        .corr(df_interpolated[cat2])
    )
...
def plot_pair_corr(df, cat1, cat2, filename_prefix):
    ...
    ax[2].axvline(
        np.argmax(np.abs(rs)), color="r", linestyle="--", label="Peak synchrony"
    )
...
def plot_total_multi_corr(cat_list, win_list, drop_reg_li):
    path_li, window_li, norm_flag_li = config()
    path_li, window_li, norm_flag_li = multi_data_config(
        path_li, norm_flag_li, cat_list, win_list
    )
    df_merged, header_list = build_merged_df(
        path_li, window_li, norm_flag_li, drop_reg_li, False
    )
    ...
    ax.set(
        xlabel="Date",
        ylabel="Rate",
        title=f"Overall Pearson r = {np.round(overall_pearson_r,2)}",
    )
